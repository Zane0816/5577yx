...
namespace Game.Manager
{
    public class Game_Qjll : IGame
    {
        ...
        public string Login(int UserId, int ServerId, int IsPC)
        {
            ...
            Sign = DESEncrypt.Md5("name=" + gu.UserName + "&platid=" + gc.AgentId + "&platuid=" + gu.Id + "&sid=" + gs.Id + "&tm=" + tstamp + gc.LoginTicket, 32);               //获取验证参数
            string LoginUrl = "http://" + gs.ServerNo + "." + gc.LoginCom + "?name=" + gu.UserName + "&platid=" + gc.AgentId + "&platuid=" + gu.Id + "&sid=" + gs.Id + "&tm=" + tstamp + "&sig=" + Sign;       //生成登录地址
            return LoginUrl;
        }
